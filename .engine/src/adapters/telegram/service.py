...
class TelegramService:
    ...
    async def init_bot_only(self):
        """Initialize only the bot client (no polling, no handlers).

        Used by MCP tools running in separate process from FastAPI.
        Creates minimal Application just for sending messages.
        """
        if not self.bot_token:
            logger.warning("TELEGRAM_BOT_TOKEN not set - cannot initialize bot")
            return False

        if not self.authorized_user_id:
            logger.warning("TELEGRAM_USER_ID not set - cannot initialize bot")
            return False
        ...
        # Build minimal Application (just for bot client)
        self.application = (
            Application.builder()
            .token(self.bot_token)
            .build()
        ...
        # Initialize the application (creates bot instance)
        await self.application.initialize()

        logger.info("Telegram bot client initialized (send-only mode)")
        return True
    ...
    async def priorities_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            from modules.priorities import PrioritiesService
    ...
    async def _handle_spawn_spec(self, update: Update, role: str, spec_text: str):
        ...
        try:
            # Write spec to conversations directory
            import uuid
            ...
            spec_path = repo_root / "Desktop" / "conversations" / spec_filename

            # Write spec
            spec_path.write_text(spec_text)

            # Spawn specialist via team tool (import inline to avoid circular deps)
            from adapters.life_mcp.tools.core import team
    ...
    async def _watch_chief_transcript(self):
        """Watch Chief's transcript across session resets.
        
        Uses two concurrent tasks:
        1. Event streaming task - forwards Claude's responses to Telegram
        2. Session check task - detects Chief resets every 10 seconds
        
        When a session change is detected, both tasks are cancelled and restarted
        with the new transcript path.
        """
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # Detect session change (reset happened)
                if chief_session.session_id != current_session_id:
                    logger.info(f"Chief session changed: {current_session_id} -> {chief_session.session_id}")
                    current_session_id = chief_session.session_id
                ...
                # Create a cancellation event for this session's tasks
                session_changed = asyncio.Event()
                
                # Task 1: Stream transcript events
                async def stream_events():
                    """Stream transcript events and forward to Telegram."""
                    try:
                        async for event in stream_transcript(
                            ...
                            if self._stop_event.is_set() or session_changed.is_set():
                                break
                            
                            # Forward text events to Telegram
                ...
                # Task 2: Periodic session check
                async def check_session():
                    """Check every 10 seconds if Chief session changed."""
                    try:
                        while not self._stop_event.is_set():
                            await asyncio.sleep(10)
                            
                            fresh_session = self._get_chief_session()
                            if fresh_session and fresh_session.session_id != current_session_id:
                                logger.info(f"Session change detected: {current_session_id} -> {fresh_session.session_id}")
                                session_changed.set()
                                break
                    except asyncio.CancelledError:
                        pass
                ...
                check_task = asyncio.create_task(check_session())
                
                # Wait for either task to complete (or session change)
                done, pending = await asyncio.wait(
                    [stream_task, check_task],
                ...
                # If session changed, loop will restart with new session
                if session_changed.is_set():
                    continue
                    
                # If we got here without session change, something else happened
