...
class ClaudeStatus:
    ...
    context_full: bool = False  # "Context limit reached" - Claude is stuck
...
def capture_pane(pane_target: str, lines: int = 15) -> Optional[str]:
    """Capture tmux pane content.

    Args:
        pane_target: Tmux pane identifier (e.g., "life:chief", "%21")
        lines: Number of lines to capture (default 15 - need enough to see active task)
...
def parse_claude_status(pane_content: str) -> ClaudeStatus:
    ...
    # 1b. Context FULL - Claude is stuck, needs emergency reset
    # Pattern: "Context limit reached · /compact or /clear to continue"
    if re.search(r'Context limit reached', pane_content, re.IGNORECASE):
    ...
    # Example: "✻ Deciphering… (thought for 10s)"
    # Icon is always non-alphanumeric (✳ ✢ ✽ ⏺ etc), followed by space, then task name, then ellipsis
    # Active task appears 6-8 lines from bottom, right above separator/prompt section
    # To avoid matching old messages, only check specific line positions
    lines = pane_content.strip().split('\n')
    if len(lines) >= 8:
        # Check lines 6-8 from bottom (0-indexed: -6, -7, -8)
        for line_idx in [-8, -7, -6]:
            try:
                line = lines[line_idx]
                # Match pattern: non-alphanumeric icon, space, task name, ellipsis
                task_match = re.match(r'^\s*([^\w\s])\s+([^…]+)…', line)
                if task_match:
                    status.active_task = task_match.group(2).strip()
    ...
    try:
        ...
        for line in result.stdout.strip().split('\n'):
            ...
            if content:
                ...
                if status.model or status.context_percent_used is not None:
