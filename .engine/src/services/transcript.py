...
def parse_transcript_line(line: str) -> Optional[TranscriptEvent]:
    """Parse a single JSONL line from a transcript file.

    Returns None for non-content events (queue-operation, file-history-snapshot, etc.)
    """
    try:
    ...
    if raw_type == "user":
        ...
        # Process content blocks - each line might have multiple
        # We'll return the first significant one
...
def format_event_for_sse(event: TranscriptEvent, include_thinking: bool = True) -> Optional[Dict[str, Any]]:
    ...
    if event.event_type == "user_message":
        ...
        # Truncate large inputs for SSE
        if event.tool_input:
            input_str = json.dumps(event.tool_input)
            if len(input_str) > 500:
                result["toolInput"] = {"_truncated": True, "preview": input_str[:500]}
            else:
