...
from utils.tmux import inject_message, send_escape_to_pane, display_message
...
class MessagingService:
    ...
    def display(
        ...
        Use for informational notifications (worker completions, pings).
...
3. Call `reset(summary="...", path="Desktop/working/[your-work].md", reason="context_low")`
...
1. Document current work in Desktop/working/[your-file].md
...
Your work continues in a fresh Chief session after the mission."""
    ...
    def notify_ping(self, from_session: str, message: str) -> bool:
        """Send ping notification to Chief.

        Uses tmux injection so Claude actually sees the message.
        """
        target = MessageTarget.by_role("chief")
        short_id = from_session[:8] if from_session else "unknown"

        ping_message = f"[{_timestamp()}] [CLAUDE OS SYS: NOTIFICATION]: Ping from {short_id}\n\n{message}"
