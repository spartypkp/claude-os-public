...
def _get_preferred_calendars() -> List[str]:
    """Get preferred calendars from config (lazy import to avoid circular deps)."""
    from apps.calendar import get_preferred_calendars
    return get_preferred_calendars()


def _get_aliases() -> Dict[str, str]:
    """Get calendar aliases from config (lazy import to avoid circular deps)."""
    from apps.calendar import get_aliases
    return get_aliases()


def _get_default_calendar() -> str:
    """Get default calendar from config (lazy import to avoid circular deps)."""
    from apps.calendar import get_default_calendar
...
class AppleCalendarAdapter(CalendarAdapter):
    ...
    def __init__(self, preferred_only: bool = False):
        """Initialize Apple Calendar adapter.
        
        Args:
            preferred_only: If True, filter to _get_preferred_calendars() only.
        """
    ...
    def get_calendars(self) -> List[CalendarInfo]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if self._preferred_only and name not in _get_preferred_calendars():
                    continue
    ...
    def get_events(
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if self._preferred_only and cal_name and cal_name not in _get_preferred_calendars():
                    continue
