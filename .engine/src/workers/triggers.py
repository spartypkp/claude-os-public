...
# Default timezone: Pacific - all trigger times are Pacific
...
class TriggerService:
    ...
    async def inject_trigger_message_async(self, message: str, source: str = "TRIGGER") -> bool:
        """Async wrapper to avoid blocking the event loop."""
        return await asyncio.to_thread(self.inject_trigger_message, message, source)
    ...
    async def check_and_fire_triggers(self):
        ...
        try:
            ...
            record_worker_latency("triggers.check", elapsed_ms, errored)
    ...
    async def _check_scheduled_trigger(self, trigger: dict):
        ...
        if time_spec == "08:00":
            message = (
                ...
                "Use /chief-morning-brief skill, then send result to the user via Telegram."
            )
            source = "MORNING-BRIEF"
        elif time_spec == "21:00":
            message = (
                "[EVENING-CHECKIN] Time for evening check-in. "
                "Use /chief-evening-checkin skill, then send result to the user via Telegram."
            )
        ...
        # Inject the message
        success = await self.inject_trigger_message_async(message, source=source)
    ...
    async def _check_calendar_trigger(self, trigger: dict):
        ...
        for event in events:
            ...
            message = (
                ...
                f"Decide if the user needs context. If important (interviews, mocks, "
                f"meetings with contacts), send brief via Telegram. If casual, skip."
            )

            success = await self.inject_trigger_message_async(message, source="PRE-EVENT")
        ...
        try:
            while not stop_event.is_set():
                ...
                try:
                    ...
                    elapsed_ms = (time.perf_counter() - start) * 1000
                    record_worker_latency("triggers.tick", elapsed_ms, errored)
