...
(async () => {
  ...
  try {
    ...
    for (const app of CORE_APPS) {
      await page.goto(`${process.env.DASHBOARD_URL || 'http://localhost:3000'}/desktop`, { waitUntil: 'load', timeout: 30000 });
    ...
    for (const app of CUSTOM_APP_ROUTES) {
      await page.goto(`${process.env.DASHBOARD_URL || 'http://localhost:3000'}${app.route}`, { waitUntil: 'load', timeout: 15000 });
  ...
  if (pageErrors.length > 0) {
    console.log(`\n  WARNING: ${pageErrors.length} page error(s) (non-fatal):`);
